...
BEGIN_MESSAGE_MAP(Tab_Debug, CDialogEx)
	...
	ON_EN_CHANGE(ID1_EC_KEYSTRING, &Tab_Debug::OnEnChangeEcKeystring)
END_MESSAGE_MAP()
...
void Tab_Debug::OnBnClickedBtnLedSetting()
{
	...
	if (info.GetLength() == 0){
		MessageBoxW(CString("请输入要显示的信息"));
		return ;
	}
	memcpy(data, info.GetBuffer(), info.GetAllocLength());//CString => char*

	((CEdit*)GetDlgItem(ID1_EC_LED_DIGIT))->GetWindowTextW(digit);
	if (digit.GetLength() == 0){//默认一位小数
		digit = "1";
	}
	point = (unsigned char)digit.GetAt(0);

	int res = LED(data, info.GetAllocLength(), point);
...
{
	UpdateData(TRUE);
	m_uc_key_type = 10 + m_key_type;
}

void Tab_Debug::OnEnChangeEcKeystring()
{}
// [密钥]输入框失去焦点
...
#ifdef __DEBUG_MODE__
	CString msg("");
	msg.AppendFormat(_T("section_num: %d, block_num: %d, key_type:%d, key: %s"), m_section_num, m_block_num, m_uc_key_type, HexStr2CStr(m_key, 6));
...
void Tab_Debug::OnBnClickedBtnWriteBlock()
{
	...
	//写块
	int res = write_block(m_block_num, m_section_num, m_uc_key_type, m_key, data, data_len);
